...
export default class Weatherly extends Component {
  ...
  getWeather(location) {
    ...
    $.get(`http://api.wunderground.com/api/${key}/hourly10day/conditions/q/${location}.json`).then((hourlyWeather) => {
      this.state.hourly.push(hourlyWeather);
      this.setState({
        weather: this.state.hourly,
      });
      console.log(this.state.hourly)
    }); $.get(`http://api.wunderground.com/api/${key}/forecast10day/q/${location}.json`).then((tenDayWeather) => {
